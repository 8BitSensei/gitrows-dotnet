...
namespace gitrows_dotnet
{
    public class Gitrows
    {
        ...
        public Gitrows(string? server = null, string ns = "github", string? owner = null, string? repo = null, string branch = "HEAD", string? path = null, string? user = null, string? token = null, string message = "Gitrows-Dotnet API Post", Author? author = null, Csv? csv = null, string? types = null, bool strict = false, string? @default = null)
        {
        ...
        public async Task<(string?, HttpStatusCode)> Get(string path, Dictionary<string, string>? query = null, string method = "fetch")
        {
            ...
            return (result.Item1, HttpStatusCode.NotFound);
        }
        ...
        public async Task<HttpStatusCode> Push(string path, object data, string? sha = null, string method = "PUT")
        {
            ...
            pathData.TryGetValue("ns", out var nsGroup);
            if (nsGroup == null)
                return HttpStatusCode.BadRequest;
            ...
            switch (nsGroup!.Value)
            {
                ...
                case "github":
                    ...
                    break;
                default:
                    throw new Exception("Invalid ns");
            }

            using (var stream = new MemoryStream())
            {
                await JsonSerializer.SerializeAsync(stream, body, body.GetType());
                stream.Position = 0;
                using var reader = new StreamReader(stream);
                serialisedData = await reader.ReadToEndAsync();
        ...
        async Task<(GitResponse?, HttpStatusCode)> Pull(string url)
        {
            ...
            if (!string.IsNullOrEmpty(_user) && !string.IsNullOrEmpty(_token) && !string.IsNullOrEmpty(_repo.Ns))
                _sharedClient.DefaultRequestHeaders.Add("Authorization", Convert.ToBase64String(Encoding.UTF8.GetBytes($"{_user}:{_token}")));
